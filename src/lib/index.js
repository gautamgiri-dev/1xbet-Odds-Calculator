 ...
 */
export async function fetchLiveMatches(domain, sportId) {
    const url = combineUrls(domain, `/service-api/LiveFeed/Get1x2_VZip?sports=${sportId}&count=20000&lng=en&gr=413&mode=4&country=71&partner=71&getEmpty=true&virtualSports=true&noFilterBlockEvent=true`);
    ...
    return jsonData.Value.map((/** @type {any} */ x) => ({
        ...
        sportId: sportId
    })).filter((/** @type {import("../app").LiveMatches} */ x) => !get(configurations).excludedLeagues.includes(x.league))
}
 ...
 */
export function getMatchRelativeURL(sportName, leagueName, leagueId, matchId, team1Name, team2Name) {
...
export async function SaveConfigurations() {
    ...
    await writeTextFile("app.conf", JSON.stringify(state), {dir: BaseDirectory.AppLocalData});
}
...
export async function LoadConfigurations() {
    ...
    if(!await exists('app.conf', {dir: BaseDirectory.AppLocalData})) await SaveConfigurations();

    const stateRaw = await readTextFile("app.conf", {dir: BaseDirectory.AppLocalData});
    const state = JSON.parse(stateRaw);

    configurations.set(state);
}